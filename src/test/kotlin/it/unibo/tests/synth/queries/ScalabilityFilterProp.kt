...
class ScalabilityFilterProp(val graph: Graph, val thr: Int) : Querying {
    override val queryId = "SynthFilterProp_foo_$thr"
    override fun runQuery(threads: Int, queryMode: QueryMode): QueryResultData {
        val simplePattern =
            listOf(
                ...
                null,
                Step(properties = listOf(Filter(VALUE, Operators.LTE, (thr / 2).toLong())))
            )
        val result: List<Any>
